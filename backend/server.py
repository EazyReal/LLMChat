...
# Handle a posr request to /start_call
# the endpoint should receive a starting prompt for the agent
# and response the caller a call id
@app.post("/start_call")
async def start_call(request: Request):
    try:
        post_data = await request.json()
        agent_prompt = post_data["agent_prompt"]
        llm_client.agent_prompt = agent_prompt
        print(f"Agent prompt: {agent_prompt}")
        web_call_response = retell.call.create_web_call(
            agent_id=agent_id,
        )
        # convert response to json
        json_data = web_call_response.__dict__
        print(json_data)
